...
countspos={}
for word in V:
    # Scaling according to polarity score
    score=s.polarity_scores(word)
    if word in neg.keys():
        countsneg[word]=neg[word]
        if score["compound"]!=0:
            if score["compound"]<0:        
                countsneg[word]*=(abs(score["compound"])+1)
            elif score["compound"]>0:
                countsneg[word]*=(1-abs(score["compound"]))
    else:
        countsneg[word]=0
    if word in pos.keys():
        ...
        if score["compound"]!=0:
            if score["compound"]>0:        
                countspos[word]*=(abs(score["compound"])+1)
            elif score["compound"]<0:
                countspos[word]*=(1-abs(score["compound"]))
    else:
        countspos[word]=0
    #Scaling according to the sentiment 
    # score=s.polarity_scores(word)
    # k=4
    # if word in neg.keys():
    #     countsneg[word]=neg[word]
